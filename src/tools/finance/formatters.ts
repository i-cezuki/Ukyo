...
export function formatJpyAmount(yen: unknown): string {
  const numeric = typeof yen === 'number' ? yen : Number(yen);
  ...
  if (abs >= 1e12) {
    return `${sign}${(abs / 1e12).toFixed(1)}兆円`;
  }
  if (abs >= 1e8) {
    return `${sign}${Math.round(abs / 1e8)}億円`;
  }
  if (abs >= 1e4) {
    return `${sign}${Math.round(abs / 1e4)}万円`;
  }

...
Chat with Ukyo through WhatsApp by linking your phone to the gateway. Messages you send to yourself (self-chat) are processed by Ukyo and responses are sent back to the same chat.
...
- Ukyo installed and working (see main [README](../../../../README.md))
- WhatsApp installed on your phone
- Your phone connected to the internet

## 🔗 How to Link WhatsApp

Link your WhatsApp account to Ukyo by scanning a QR code:
...
After linking, you'll be asked how you want to use Ukyo:

### Option 1: Self-chat (personal phone)

Use your own WhatsApp to talk to Ukyo by messaging yourself. The linked phone number is added to `allowFrom` and self-chat mode is activated automatically.

### Option 2: Dedicated bot phone

If Ukyo has its own phone number (e.g. a separate SIM), choose this option and enter the phone number(s) allowed to message it. The gateway will be configured with `dmPolicy: "allowlist"` so other people can DM the bot.
...
```
[whatsapp] Connected
Ukyo gateway running. Press Ctrl+C to stop.
```

The gateway will now listen for incoming WhatsApp messages and respond using Ukyo.
...
3. Send a message like "トヨタ(7203)の最新株価を教えて"
4. You'll see a typing indicator while Ukyo processes
5. Ukyo's response will appear in the chat

**Example conversation:**
```
You: キーエンス(6861)のPBRと配当利回りを教えて
Ukyo: キーエンスのPBRは ... 倍、配当利回りは ... % です。直近株価と財務サマリーをもとに計算しています。
```
...
**Bot phone configuration** (dedicated Ukyo phone, others message it):
```json
{
  "gateway": {
    "accountId": "default",
...
| Setting | Description |
|---------|-------------|
| `channels.whatsapp.allowFrom` | Phone numbers allowed to message Ukyo (E.164 format) |
...
Ukyo can participate in WhatsApp group chats, responding only when @-mentioned.
...
| `groupAllowFrom` | Which groups Ukyo can participate in (`["*"]` for any) |

You don't need to list individual group members — when `groupPolicy` is `"open"`, Ukyo will respond to @-mentions from anyone in any group it's added to.

### Usage

1. Add Ukyo's WhatsApp number to a group
2. Send messages normally — Ukyo stays silent
3. @-mention Ukyo (tap `@` and select from the picker) to get a response
4. Ukyo sees recent group messages for context, so it can follow the conversation
...
2. **Unlink from WhatsApp:**
   ...
   - Tap on the Ukyo device and select **Log Out**

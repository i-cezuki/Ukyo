...
async function promptSetupMode(cfg: GatewayConfig, linkedPhone: string): Promise<void> {
  ...
  try {
    ...
    console.log('');
    console.log('How will you use Ukyo with WhatsApp?');
    console.log('  1) Self-chat  — message yourself to talk to Ukyo');
    console.log('  2) Bot phone  — this is a dedicated Ukyo phone, others message it');
    ...
    console.log('');
    console.log('Enter the phone number(s) allowed to message Ukyo (E.164 format, e.g. +15551234567).');
...
async function run(): Promise<void> {
  ...
  console.log('Ukyo gateway running. Press Ctrl+C to stop.');
